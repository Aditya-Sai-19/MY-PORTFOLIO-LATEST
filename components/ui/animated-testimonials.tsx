...
export const AnimatedTestimonials = ({
  ...
  // Random rotation is resolved after mount only, so the server render and
  // the first client render agree (fixes the hydration mismatch caused by
  ...
  useEffect(() => {
    ...
    testimonials.forEach((_, index) => {
      next[index] = Math.floor(Math.random() * 21) - 10;
    });
    setRotations(next);
  }, [testimonials]);
  ...
  return (
    <div className={cn("mx-auto max-w-sm px-4 py-20 md:max-w-4xl md:px-8 lg:px-12", className)}>
