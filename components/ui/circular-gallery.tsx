...
export interface GalleryItem {
  image: string;
  text: string;
  /** Optional link opened when the item is clicked (e.g. certificate verify page). */
  href?: string;
}
...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          // Object-contain: the whole certificate always fits inside the card,
          // letterboxed on the transparent background when aspects differ.
          float planeAspect = uPlaneSizes.x / max(uPlaneSizes.y, 0.0001);
          float imgAspect = uImageSizes.x / max(uImageSizes.y, 0.0001);
          vec2 uv = vUv;
          if (planeAspect > 0.01 && imgAspect > 0.01) {
            if (planeAspect > imgAspect) {
              float s = imgAspect / planeAspect;
              uv.x = vUv.x * s + (1.0 - s) * 0.5;
            } else {
              float s = planeAspect / imgAspect;
  ...
  onResize(
    ...
    // Cards match the dominant certificate document aspect (~1.91:1), so the
    // preview fills the card edge-to-edge instead of letterboxing inside it.
    this.plane.scale.y =
      (this.viewport.height * (500 * this.scale)) / this.screen.height;
    this.plane.scale.x =
      (this.viewport.width * (955 * this.scale)) / this.screen.width;
...
class App {
  ...
  constructor(
    ...
    this.mediaHrefs = (items ?? []).map((i) => i.href);
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    if (Math.abs(distance) > 6) this.hasDragged = true;
  ...
  onClick(e: MouseEvent) {
    ...
    if (this.mediaHrefs.length === 0) return;

    const rect = this.container.getBoundingClientRect();
    // Screen offset -> world-space offset under the same projection.
    const worldX =
    ...
    if (!closest) return;
    const href = this.mediaHrefs[closest.index % this.mediaHrefs.length];
